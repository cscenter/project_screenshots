      ...
      for (let url of urls_list) {
          if (url) {
            ...
            try {
              ...
              await page.screenshot({path: file_name + "_helv_" + screenshot_extension , fullPage: true});
              var html = await page.content();
              ys_html = await html.replace(/font-family:/g, "font-family:\"YS Text\",");
              await page.setContent(ys_html);
              await page.screenshot({path: file_name + "_ys_" + screenshot_extension , fullPage: true});
              arial_html = await html.replace(/font-family:/g, "font-family:\"Arial\",");
              await page.setContent(arial_html);
              await page.screenshot({path: file_name + "_arial_" + screenshot_extension , fullPage: true});
              ss_html = await html.replace(/font-family:/g, "font-family:\"sans-serif\",");
              await page.setContent(ss_html);
              await page.screenshot({path: file_name + "_sansserif_" + screenshot_extension , fullPage: true});
              ubuntu_html = await html.replace(/font-family:/g, "font-family:\"ubuntu\",");
              await page.setContent(ubuntu_html);
              await page.screenshot({path: file_name + "_ubuntu_" + screenshot_extension , fullPage: true});
              tnr_html = await html.replace(/font-family:/g, "font-family:\"Times New Roman\",");
              await page.setContent(tnr_html);
              await page.screenshot({path: file_name + "_tnr_" + screenshot_extension , fullPage: true});
