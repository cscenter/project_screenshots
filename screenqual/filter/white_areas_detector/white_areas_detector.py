...
class WhiteAreasAnalyser(ScreenshotAnalyser):
    ...
    def execute(self, screenshot):
        ...
        dp[0, 0] = self.DpElementInfo()

        for i in range(1, w):
            if thresh[0, i] == 0:
                dp[0, i].width = dp[0, i - 1].width + 1
            else:
                dp[0, i] = self.DpElementInfo()

        for i in range(1, h):
            if thresh[i, 0] == 0:
                dp[i, 0].height = dp[i - 1, 0].height + 1
            else:
                dp[i, 0] = self.DpElementInfo()
        ...
        for i in range(1, h):
            for j in range(1, w):
                if thresh[i, j] == 0:
                    dp[i, j].width = min(dp[i, j - 1].width,
                                         dp[i - 1, j - 1].width) + 1
                    dp[i, j].height = min(dp[i - 1, j].height,
                                          dp[i - 1, j - 1].height) + 1
